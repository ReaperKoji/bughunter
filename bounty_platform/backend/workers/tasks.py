...
def run_nuclei_scan(scan_job_id: int) -> dict:
    ...
    templates = "cves,misconfiguration/cloud"
    cmd = [
        "nuclei",
        "-u",
        f"https://{target}",
    ...
    for line in out.splitlines():
        ...
        repro = f"nuclei -u {endpoint} -id {row.get('template-id','')}"
        vuln = Vulnerability.objects.create(
            program=job.program,
            subdomain=job.subdomain,
            title=title,
            category=str(row.get("template-id", "")),
            severity=sev if sev in {"info", "low", "medium", "high", "critical"} else "low",
            confidence=70.0,
            tool="nuclei",
            endpoint=endpoint,
...
def run_ai_triage(vulnerability_id: int) -> dict:
    ...
    if isinstance(raw.get("curl-command"), str):
        body = raw.get("curl-command", "")
